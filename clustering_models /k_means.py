...
class MyKMeans():
    ...
    def __init__(self, n_clusters: int = 3, max_iter: int = 10, n_init: int = 3, random_state: int = 42) -> None:
        ...
        Args:
            ...
            max_iter: Максимальное количество итераций для уточнения координат центроидов.
            n_init: Число итераций всего алгоритма. 
    ...
    def _assign_cluster(self, X: pd.DataFrame, clusters: dict) -> list:
        ...
        Args:
            X : Датасет для кластеризации.
            clusters: Словарь, определяющий кластеры.
    ...
    def _get_wcss(self, X: pd.DataFrame, clusters_version: dict) -> float:
        ...
        Args:
            X: Датасет для кластеризации. 
            clusters_version: Рассматриваемый вариант кластеризации.

        Returns:
            WCSS для рассматриваемого датасета и варианта кластеризации.
