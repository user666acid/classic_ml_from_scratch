class MyAgglomerative():
    ...
    def _init_clusters(self, X: pd.DataFrame) -> dict:
        ...
        Returns:
            dict: Словарь, определяющий кластеры. 
    ...
    def _find_closest_clusters(self) -> tuple:
        """ Выполняет поиск ближайших кластеров. 

        Returns:
            tuple: Массив содержащий номера ближайших кластеров и индексы объединения их точек. 
    ...
    def _update_clusters(self, clusters_to_merge: tuple, points: list) -> dict:
        """ Обновляет кластеры, объединяя ближайшие кластеры. 

        Args:
            clusters_to_merge (tuple): Массив с номерами объединяемых кластеров.
            points (list): Список с индексами точек объединяемых кластеров. 

        Returns:
            dict: Словарь с обновленными кластерами. 
    ...
    def _get_distance(self, cluster_i: np.array, cluster_j: np.array) -> float:
        ...
        Args:
            cluster_i, cluster_j (np.array): Кластеры 

        Returns:
            float: Расстояние между кластерами.
